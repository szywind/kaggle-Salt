import cv2, random
import numpy as np
from constants import PAD_FRONT, PAD_END
...
def compute_center_pad(H,W, factor=32):
    ...
    return PAD_FRONT, PAD_END, PAD_FRONT, PAD_END


def do_center_pad_to_factor(image, factor=32, borderMode=cv2.BORDER_REPLICATE):
    H,W = image.shape[:2]
    dy0, dy1, dx0, dx1 = compute_center_pad(H,W, factor)

    image = cv2.copyMakeBorder(image, dy0, dy1, dx0, dx1, borderMode)
                               #cv2.BORDER_CONSTANT, 0)
    return image


def do_center_pad_to_factor2(image, mask, factor=32, borderMode=cv2.BORDER_REPLICATE):
    image = do_center_pad_to_factor(image, factor, borderMode)
    mask  = do_center_pad_to_factor(mask, factor, borderMode)
    return image, mask
...
def do_random_shift_scale_crop_pad2(image, mask, limit=0.10, keep_aspect=False):
...
def do_shift_scale_rotate2(image, mask,
...
def do_elastic_transform2(image, mask, grid=32, distort=0.2):
    borderMode=cv2.BORDER_REPLICATE # cv2.BORDER_REFLECT_101
    height, width = image.shape[:2]
...
def train_augment(image,mask):
    ...
    if np.random.rand() < 0.5:
        image, mask = do_random_shift_scale_crop_pad2(image, mask, 0.1, keep_aspect=True)
    ...
    if c == 3:
        image, mask = do_center_pad_to_factor2(image, mask, factor=32, borderMode=cv2.BORDER_REFLECT_101)
    if c == 4:
        image, mask = do_center_pad_to_factor2(image, mask, factor=32, borderMode=cv2.BORDER_REFLECT)
